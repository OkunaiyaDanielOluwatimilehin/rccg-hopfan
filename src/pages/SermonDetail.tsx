...
export default function SermonDetail() {
  ...
  const isVisibleSermon = (value: Pick<Sermon, 'published_at' | 'sermon_date' | 'status'>) => {
    if (value.status === 'draft') return false;
    const publishedDate = value.published_at || value.sermon_date;
  ...
  useEffect(() => {
    async function fetchSermon() {
      ...
      try {
        ...
        if (!isVisibleSermon(data as Sermon)) {
          setSermon(null);
        } else {
          setSermon(data);
        }
    ...
    const loadRelatedSermons = async () => {
      ...
      try {
        ...
        const merged = results
          ...
          .filter((item) => isVisibleSermon(item as Sermon))
      ...
      <section className="w-full px-3 sm:px-10 md:px-16 pt-6 sm:pt-10 relative z-10">
        <div className="max-w-[88rem] mx-auto space-y-8 sm:space-y-10">
          ...
          <div className="bg-white border border-stone-200 shadow-sm">
            <div className={`grid gap-0 border-b border-stone-200 bg-stone-50/70 ${availableTabs.length <= 1 ? 'grid-cols-1' : availableTabs.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>
              ...
              {activeTab === 'video' && sermon.video_url ? (
                ...
                <div className="space-y-6 sm:space-y-8">
                  ...
                  <AudioPlayer
                    ...
                    subtitleRight={format(new Date(sermon.published_at || sermon.sermon_date), 'MMMM d, yyyy')}
                    topActions={
                      <>
                        <button
                          type="button"
