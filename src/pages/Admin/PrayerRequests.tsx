...
import { Loader2, MessageSquareHeart, ShieldAlert, Send } from 'lucide-react';
...
export default function AdminPrayerRequests() {
  ...
  async function fetchAll() {
    ...
    setError(null);
    try {
      const fullSelect = 'id,message,is_private,created_at,status,assigned_team,assigned_person,assigned_person_id';
      const fallbackSelect = 'id,message,is_private,created_at,status,assigned_team,assigned_person';
      const requestsRes = await supabase.from('prayer_requests').select(fullSelect).order('created_at', { ascending: false });
      if (requestsRes.error && isMissingColumnError(requestsRes.error, 'assigned_person_id')) {
        const fallbackRes = await supabase.from('prayer_requests').select(fallbackSelect).order('created_at', { ascending: false });
        ...
        setRequests((requestsRes.data || []) as PrayerRequest[]);
      }

      const profilesRes = await supabase
        .from('profiles')
        .select('id,full_name,email,role')
        .in('role', ['admin', 'prayer', 'follow_up'])
        .order('full_name', { ascending: true });
      if (profilesRes.error) throw profilesRes.error;
      setProfiles((profilesRes.data || []) as ProfileRow[]);
    } catch (err: any) {
      console.error('Error fetching prayer requests:', err);
      setError(err?.message || 'Could not load prayer requests.');
      setRequests([]);
      setProfiles([]);
    } finally {
      setLoading(false);
  ...
  async function updateRequest(id: string, patch: Partial<PrayerRequest>) {
    setSavingId(id);
    try {
      const { error } = await supabase.from('prayer_requests').update(patch).eq('id', id);
      if (error) {
        ...
        const fallback = await supabase.from('prayer_requests').update(fallbackPatch).eq('id', id);
        if (fallback.error) throw fallback.error;
      }
      setRequests((current) => current.map((item) => (item.id === id ? { ...item, ...patch } : item)));
    } catch (err: any) {
      console.error('Error updating prayer request:', err);
      setError(err?.message || 'Could not update prayer request.');
    } finally {
      setSavingId(null);
    }
  }
  ...
  const getPersonOptions = (team?: string | null) => {
    ...
    return teamMembers.filter((member) => member.role === 'prayer');
  };
  ...
  const visibleRequests = useMemo(() => {
    ...
    return requests.filter((request) =>
      ...
      request.assigned_team === 'prayer_team'
    );
  }, [requests, role, user?.id, currentProfile?.full_name]);

  const notifyAssignment = async (request: PrayerRequest, patch: Partial<PrayerRequest>) => {
    ...
    await dispatchRequestAssignment({
      ...
      body: 'A prayer request has been assigned to you in the church dashboard.',
  ...
  const handleAssign = async (request: PrayerRequest) => {
    const patch: Partial<PrayerRequest> = {
      status: 'tagged',
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        <h1 className="text-3xl font-serif font-bold text-primary tracking-tight">Prayer Requests</h1>
        <p className="text-stone-500 text-sm font-light">View prayer requests submitted from the home page.</p>
      </div>
      ...
      <div className="bg-white border border-stone-200 shadow-sm">
        ...
        {loading ? (
          <div className="p-8 text-stone-500">Loading prayer requests...</div>
        ) : visibleRequests.length === 0 ? (
          <div className="p-8 text-stone-500">No prayer requests yet.</div>
        ) : (
            ...
            {visibleRequests.map((request) => (
              <article key={request.id} className="p-6 space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h3 className="text-lg font-bold text-primary">Prayer Request</h3>
                ...
                <div className="grid md:grid-cols-3 gap-4 text-sm">
                  <div className="p-4 bg-stone-50 border border-stone-100 md:col-span-2">
                    ...
                    {shouldRedactContent ? (
                      <p className="leading-relaxed text-stone-700">Request content hidden from admin inbox. Forward or tag the relevant team instead.</p>
                    ) : (
                      <p className="leading-relaxed text-stone-700 whitespace-pre-wrap">
