...
import { Play, Calendar as CalendarIcon, ArrowRight, Clock, Users, Heart, Image as ImageIcon, Music, Mic2, ChevronLeft, ChevronRight, MapPin, MessageSquare, HandHeart, Sparkles, Mail, Quote, Share2, ExternalLink } from 'lucide-react';
...
export default function Home() {
    ...
    async function fetchAllData() {
      try {
        const [
          ...
          supabase.from('events').select('*').limit(4).order('event_date', { ascending: true }),
  ...
  return (
      ...
      {events.length > 0 && (
        <section className="w-full px-4 sm:px-8 md:px-16 py-6 sm:py-12">
          <div className="relative overflow-hidden bg-primary h-[300px] sm:h-[400px] group">
            <motion.div 
              animate={{ x: [0, -100 * (events.length - 1) + '%'] }}
              ...
              {events.map((event, idx) => (
                <div key={idx} className="min-w-full h-full relative">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center px-6 sm:px-12 md:px-24 text-white">
                    <span className="text-xs sm:text-sm text-accent font-bold uppercase tracking-widest mb-2 sm:mb-4">Latest Event</span>
                    <h2 className="text-2xl sm:text-4xl md:text-6xl font-serif font-bold mb-4 sm:mb-6">{event.title}</h2>
                    <Link 
                      to={`/events/${event.id}`}
      ...
      <section id="support-giving" className="w-full px-8 md:px-16 py-32 bg-stone-50/50 border-t-2 border-stone-200">
        ...
        <div className="grid md:grid-cols-3 gap-12">
          ...
          >
            ...
            <p className="text-stone-300 mb-10 text-xl leading-relaxed font-light">Support the work of God through your tithes, offerings, and special donations.</p>
            <div className="flex items-center gap-3 text-accent font-bold text-xl">
              Give Online <ArrowRight className="w-7 h-7 group-hover:translate-x-1 transition-transform" />
            </div>
          </motion.button>
        </div>
      </section>
      ...
      <section id="upcoming-events" className="w-full px-8 md:px-16 py-32 bg-white border-t-2 border-stone-200">
        <div className="text-center max-w-4xl mx-auto mb-20">
      ...
      <section className="w-full px-8 md:px-16 py-32 bg-white border-t-2 border-stone-200">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          <div className="max-w-3xl">
            ...
            <p className="text-2xl text-stone-500 mt-6 font-light">Stay updated with our church calendar and never miss a moment of fellowship.</p>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10">
          {(events.length > 0 ? events : [
            { id: '1', title: "Night of Wonders", event_date: "2026-03-28", event_time: "10:00 PM", location: "Main Sanctuary", category: "Prayer", image_url: "https://images.unsplash.com/photo-1438029071396-1e831a7fa6d8?auto=format&fit=crop&q=80" },
            { id: '2', title: "Youth Empowerment Summit", event_date: "2026-04-05", event_time: "12:00 PM", location: "Youth Hall", category: "Youth", image_url: "https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&q=80" },
            { id: '3', title: "Easter Celebration Service", event_date: "2026-04-12", event_time: "10:00 AM", location: "Main Sanctuary", category: "Special", image_url: "https://images.unsplash.com/photo-1516280440614-37939bbacd81?auto=format&fit=crop&q=80" },
            { id: '4', title: "Bible Study", event_date: "2026-03-24", event_time: "6:00 PM", location: "Online/Church", category: "Study", image_url: "https://images.unsplash.com/photo-1529070538774-1843cb3265df?auto=format&fit=crop&q=80" }
          ]).map((event, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
      ...
      >
        <div className="space-y-8">
          <p className="text-stone-500 leading-relaxed">
            Your generosity helps us continue our mission of spreading the gospel and serving our community. Thank you for your faithful support.
          </p>
          <div className="grid grid-cols-2 gap-4">
            <button className="p-6 rounded-xl border-2 border-accent bg-accent/5 text-primary font-bold hover:bg-accent/10 transition-all text-center">
              <span className="block text-2xl mb-1">Tithes</span>
              <span className="text-xs text-stone-400 uppercase tracking-widest">10% of Income</span>
            </button>
            <button className="p-6 rounded-xl border-2 border-stone-100 hover:border-accent/30 transition-all text-center">
              <span className="block text-2xl mb-1">Offering</span>
              <span className="text-xs text-stone-400 uppercase tracking-widest">General Donation</span>
            </button>
          </div>
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-bold text-stone-400 uppercase tracking-widest ml-1">Amount ($)</label>
              <input type="number" placeholder="50.00" className="w-full p-5 rounded-xl bg-stone-50 border border-stone-200 focus:ring-2 focus:ring-accent outline-none text-primary text-2xl font-bold" />
            </div>
            <button className="w-full bg-accent text-white py-6 rounded-xl font-bold text-lg hover:bg-accent/90 transition-all shadow-xl shadow-accent/20">
              Proceed to Payment
            </button>
          </div>
          <div className="pt-6 border-t border-stone-100">
            <h4 className="font-bold text-primary mb-4">Other Ways to Give</h4>
            <div className="space-y-3 text-sm text-stone-500">
              <p><strong>Bank Transfer:</strong> RCCG HOPFAN - 0123456789 (Bank Name)</p>
              <p><strong>Zelle:</strong> giving@hopfan.org</p>
              <p><strong>Cash/Check:</strong> During any of our physical services.</p>
            </div>
          </div>
        </div>
      </Modal>
